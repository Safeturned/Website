            ...
            <div className='max-w-6xl mx-auto px-6 py-12'>
                ...
                <div className='bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-xl p-6 mb-8'>
                    <div className='flex items-center justify-between mb-4'>
                        <div>
                            <h2 className='text-2xl font-bold text-white'>{unlockedCount} / {totalCount}</h2>
                            <p className='text-slate-400 text-sm'>Achievements Unlocked</p>
                        </div>
                        <div className='text-4xl'>
                            {completionPercentage === 100 ? '🎉' : completionPercentage >= 75 ? '⭐' : completionPercentage >= 50 ? '🌟' : completionPercentage >= 25 ? '✨' : '💫'}
                        </div>
                    </div>
                    ...
                    <p className='text-center text-sm text-slate-400 mt-2'>{completionPercentage}% Complete</p>
                </div>
                    ...
                    {achievements.map((achievement) => (
                        ...
                        >
                            <div className='flex items-start gap-4'>
                                <div className={`text-5xl transition-all duration-300 ${achievement.unlocked ? 'scale-110' : 'grayscale opacity-50'}`}>
                                    {achievement.icon}
                                </div>
                                <div className='flex-1'>
                                    <h3 className={`text-xl font-bold mb-1 ${achievement.unlocked ? 'text-yellow-400' : 'text-gray-400'}`}>
                                        {achievement.title}
                                    </h3>
                ...
                {completionPercentage === 100 && (
                    <div className='mt-8 bg-gradient-to-r from-yellow-900/30 to-purple-900/30 border-2 border-yellow-500 rounded-xl p-8 text-center'>
                        ...
                        <p className='text-gray-300'>You've unlocked all achievements! You're a true Safeturned champion!</p>
                    </div>
                )}
            </div>

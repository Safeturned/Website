...
export async function POST(request: NextRequest) {
    try {
        ...
        const response = await fetch(`${apiUrl}/v1.0/files/upload/initiate`, {
            method: 'POST',
            headers,
        ...
        if (!response.ok) {
            ...
            console.error('API initiate failed:', response.status, errorText);
            return NextResponse.json(
                { error: `Initiate failed: ${response.status} ${response.statusText}` },
